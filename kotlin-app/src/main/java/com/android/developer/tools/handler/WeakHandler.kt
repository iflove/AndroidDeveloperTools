...
class WeakHandler {
    private var mCallback: Handler.Callback? = null // hard reference to Callback. We need to keep callback in memory
    ...
    constructor(callback: Handler.Callback?) {
        ...
        mExec = ExecHandler(WeakReference<Handler.Callback>(callback)) // Weak referencing inside ExecHandler
    }
    ...
    private class ExecHandler : Handler {
        ...
        override fun handleMessage(msg: Message) {
            ...
            val callback = mCallback.get() ?: // Already disposed
                    return
            callback.handleMessage(msg)
        }
    }

    internal class WeakRunnable(private val mDelegate: WeakReference<Runnable?>, private val mReference: WeakReference<ChainedRef>) : Runnable {
    ...
    internal class ChainedRef(var lock: Lock, val runnable: Runnable?) {
